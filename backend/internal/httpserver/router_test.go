...
// TestRouter_PreservesHijackerThroughChain proves http.Hijacker survives the
// full chain (AC4 names both Flusher and Hijacker). A real server connection
// supports Hijack, so the handler must both see the interface and be able to
// take over the connection.
func TestRouter_PreservesHijackerThroughChain(t *testing.T) {
	r := NewRouter(config.Config{}, applog.NewWithWriter(config.Config{Env: "production", LogLevel: "error"}, io.Discard))
	r.HandleFunc("GET /hijack", func(w http.ResponseWriter, _ *http.Request) {
		hj, ok := w.(http.Hijacker)
		if !ok {
			t.Error("handler must see http.Hijacker through the chain")
			return
		}
	...
	if string(body) != "ok" {
		t.Errorf("hijacked body = %q, want ok", body)
	}
}
