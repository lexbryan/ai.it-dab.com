...
//
// Because recovery is innermost, a panic originating in the CORS or logging
// middleware itself (not in a route handler) is not turned into an app-level
// 500; it falls through to net/http's per-connection recovery, which drops that
// one connection while the server keeps running. Those layers are deliberately
// trivial and panic-free, so this is an accepted trade-off of the ticket's
// specified middleware order.
